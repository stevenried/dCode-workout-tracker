export default class WorkoutTracker {
  static LOCAL_DATA_STORAGE_KEY = 'workout-tracker-entries'
  ...
  static html() {
    return `
        <table class="tracker" data-id="table">
            <thead>
            ...
            <tbody class="tracker__entries" data-id="table-body">
                
            </tbody>
            <tbody>
                <tr class="tracker__row tracker__row--add">
                    <td colspan="4">
                        <button type="button" class="tracker__add">
                            Add Entry &plus;
                        </button>
                    </td>
                </tr>
  ...
  static rowHTML() {
    return `
        <tr class="tracker__row">
            ...
            <td>
                <select class="tracker__workout" name="" id="">
                    ...
                    <option value="elliptical">Elliptical Machine</option>
                </select>
            </td>
            <td>
  ...
  loadEntries() {
    this.entries = JSON.parse(
      localStorage.getItem(WorkoutTracker.LOCAL_DATA_STORAGE_KEY || '[]')
    )
  }

  saveEntries() {
    localStorage.setItem(
      WorkoutTracker.LOCAL_DATA_STORAGE_KEY,
  ...
  updateView() {
    ...
    const addRow = (data) => {
      ...
      row
        .querySelector('.tracker__button')
        .addEventListener('click', () => this.deleteEntry(data))

      tableBody.appendChild(row)
    }

    tableBody.querySelectorAll('.tracker__row').forEach((row) => row.remove())
